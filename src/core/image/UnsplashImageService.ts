...
import { ContextualImage, ContextualImageRequest, ContextualImageService } from "@/core/image/ContextualImageService";
...
export class UnsplashImageService implements ContextualImageService {
    ...
    private static buildContextualImage(image: any, request: ContextualImageRequest): ContextualImage {
        const color = image.color;
        const rawUrl = image.urls.raw;
        const sizeParam = request.width && request.height ? Math.max(request.width, request.height) : 0;
